...
**Method:** we defined a *blockade-affected window*: any (month, day-of-week, hour) slot where at least one of lines 17, 19, or 22 (direction A only — see note below) ran a genuinely blocked variant that hour (missing more than 15 stops from its normal route). This gave **208 such slots** across the year (36 on Saturdays, 172 on weekdays).

This is a deliberately broader, looser rule than Section 3.1's confirmed-hours calendar, and intentionally so, not by oversight — and the reason is about what's being tested, not only about sample size. Section 3.1 needs ≥3 lines to simultaneously and unambiguously agree before an hour counts, because its job is to make a small number of highly defensible claims about exactly when a closure happened on the corridor itself. This section asks a different kind of question: lines 14 and 15 never run a detour — they don't touch the corridor — so we are not testing their own routing, but whether traffic *displaced* by a closure on 17/19/22 spills over onto the unrelated streets these two control lines actually use. That is a diffuse, network-level effect, not a precise one, so gating it on Section 3.1's strict multi-line agreement would be the wrong match for the hypothesis: any one of the three corridor lines genuinely detouring is a reasonable trigger for "conditions were plausibly disrupted nearby," which is the condition being tested here. It also, as a secondary benefit, yields enough matched trips to run a meaningful statistical test — Section 3.1's stricter bar would leave too few slots to compare distributions with any power. Both trade-offs are deliberate, documented choices in the underlying analysis; they answer different questions ("when are we certain a closure happened" vs. "is there a detectable spillover effect nearby"), not the same question twice.
...
**Table 4.** Control line travel time, blockade-affected windows vs. matched normal weeks
...
Returning to the project's central question — how can a resident know the most reliable way to leave Rechavia by bus, given recurring closures — the four findings above combine into a single practical answer. Closures are concentrated on Saturday evenings nearly year-round, plus a handful of specific spring weekday windows (3.1). During a blockade-affected window, lines 17, 19, 22, 9, and 97 all reroute rather than sit in the closure, and that reroute is faster than their normal route on average (3.2), so simply staying on one of these lines through a closure window is, on balance, not a bad choice. The exceptions are narrow but concrete: line 9 riders face a real ~12-minute delay specifically on Saturdays (3.4), and line 97's detour briefly reverses into a time cost between 14:00 and 17:00 (3.2, 3.4) — both worth avoiding at those specific times if an alternative exists. Lines 14 and 15, which don't run through the corridor at all, show no measurable slowdown at any time during a blockade-affected window (3.3) — they are the one genuinely unaffected option, though also the two lines least useful for actually leaving the neighborhood via this corridor in the first place.
